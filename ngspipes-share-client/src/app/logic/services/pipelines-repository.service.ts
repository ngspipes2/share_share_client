...
export class PipelinesRepositoryService {
    ...
    updateImage(repositoryId : number, file : any) : Promise<boolean> {
        let url = Server.UPDATE_PIPELINES_REPOSITORY_IMAGE.replace('{repositoryId}', repositoryId.toString());
    ...
    deleteImage(repositoryId : number) : Promise<boolean> {
        let url = Server.DELETE_PIPELINES_REPOSITORY_IMAGE.replace('{repositoryId}', repositoryId.toString());
